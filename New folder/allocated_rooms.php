...
if(mysqli_num_rows($result_search)==0){
    ...
    while($row_search = mysqli_fetch_assoc($result_search)){
        ...
        // Calculate floor from room number (e.g., 204 -> 2)
        $floor = floor($room_no / 100);
         ...
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $student_name = $row1['Fname']." ".$row1['Lname'];
      		...
      		echo "<tr><td>{$student_name}</td><td>{$row1['Student_id']}</td><td>{$row1['Mob_no']}</td><td>{$hostel_name}</td><td>{$room_no}</td><td>{$floor}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
